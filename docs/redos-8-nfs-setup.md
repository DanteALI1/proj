...
1. Без дополнительных дисков `/opt/share` оказался бы на корне `/dev/sda4` (~54 ГБ свободно).
2. После подключения из ESXi **3 × 2 ТБ** шару лучше разместить на новом томе (LVM ~6 ТБ) и смонтировать в `/opt/share` — см. раздел ниже.
3. Экспорт `/opt/share` при монтировании целого тома в эту точку совпадает с файловой системой тома → можно использовать `no_subtree_check`; если оставляете подкаталог на общем разделе — оставляйте `subtree_check`.
4. Наличие `/run/user/1000` означает, что есть пользователь с **UID 1000** — опции `anonuid=1000,anongid=1000` к нему корректно привязываются.
